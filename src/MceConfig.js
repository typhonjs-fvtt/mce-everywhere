...
export class MceConfig
{
   ...
   static configExtra({ basicFormats = false, code = true, contentCSS, contentStyle = '', fontFormat = true,
    fontSize = true, help = false, stripStyleFormat = true, styleFormat = true, tjsStyles = true,
     toolbar = true } = {})
   {
      ...
      const toolbarData = `${styleFormat ? 'styles |' : ''} fontfamily | ${fontSize ? 'fontsize |' : ''} table | ${
       fontFormat ? 'formatgroup |' : ''} removeformat | insertgroup | bulletgroup | save${code ? ' | code' : ''}${
        help ? ' | help' : ''}`;
    ...
    *
    * @returns {{title: string, items: [{classes: string, block: string, wrapper: boolean, title: string}]}[]} Formats.
    */
